...
#pragma message("SDL_SetWindowsMessageHook is not supported before SDL 2.0.16")
#pragma message("SDL_RenderGetD3D11Device is not supported before SDL 2.0.16")
#endif

static void SDL_SetWindowsMessageHook(SDL_WindowsMessageHook callback, void *userdata)
{
	return
}
...
func (renderer *Renderer) GetD3D11Device() (device *ID3D11Device, err error) {
	device = C.SDL_RenderGetD3D11Device(renderer.cptr())
	if device == nil {
		err = GetError()
	}
	return
